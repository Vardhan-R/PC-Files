...
def movePt(click, mouse_x, mouse_y):
    global r
    pygame.draw.circle(scrn, red, (pts[0].x, pts[0].y), 25)
    for m in pts[1: -1]: pygame.draw.circle(scrn, dark_green, (m.x, m.y), 25)
    pygame.draw.circle(scrn, grey, (pts[-1].x, pts[-1].y), 25)
    if click:
